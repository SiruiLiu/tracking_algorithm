...
% SignalTemp=zeros(1,SampleNumber);% 一个脉冲
% SignalTemp(DelayNumber+1:DelayNumber+number)=sqrt(SigPower)*Chirp;%一个脉冲的1个目标（未加多普勒速度）
% Signal=zeros(1,TotalNumber);
%
% for i=1:PulseNumber
% Signal((i-1)*SampleNumber+1:i*SampleNumber)=SignalTemp;
% end
% FreqMove=exp(j*2*pi*TargetFd*(0:TotalNumber-1)/Fs);%目标的多普勒速度*时间=目标的多普勒相移
% Signal=Signal.*FreqMove;
% SignalAll=SignalAll+Signal;
figure(1);
...
figure(4);subplot(2,1,1);plot(abs(pc_freq0));title('频域脉压结果的幅度,有前暂态点和后暂态点');
hold on;
